...
* Support disqualifications (explicitly showing that there was one
  rather just removing people and countries).
...
* Allow CSV files to specify event short and long names, and number of
  contestants for ranking, so those can vary from year to year.
...
* Update example :file:`config.ini` based on that generated by Roundup
  1.5.1.

* Add caching for scoreboard accesses at least to reduce system load.
